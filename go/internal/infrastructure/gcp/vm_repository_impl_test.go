...
func TestVMRepositoryImpl_ProgressCallback(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping long-running integration test in short mode")
	}
	...
	repo.SetProgressCallback(func() {
		callbackCalled = true
		t.Log("Progress callback invoked")
	})
	...
	// Start VM - this should trigger the progress callback
	stoppedVM, findErr := repo.FindByName(ctx, testVM)
	require.NoError(t, findErr)

	t.Log("Starting VM with progress callback...")
	startErr := repo.Start(ctx, stoppedVM)
	require.NoError(t, startErr)
	...
	// Verify callback was called during the operation
